...
COIN_THRESHOLD = 10 # After collecting 10 points,enemy gets faster
THRESHOLD_STEP = 10 # Threshold increase
...
while True:
    ...
    # Display coins collected in top right corner
    ...
    # Check for coimn collection
    collected_coins = pygame.sprite.spritecollide(P1, coins, True)
    for coin in collected_coins:
        COINS += coin.value
        # Remove coin from all_sprites when collected
        all_sprites.remove(coin)
        pygame.mixer.Sound('coin_sound.mp3').play() # chop-chop

        if COINS >= COIN_THRESHOLD:
            SPEED += 1
            COIN_THRESHOLD += THRESHOLD_STEP
